...
class Search:
    ...
    def start(self, state):
        ...
        α = -inf
        β = inf
        return self.negamax(state, self.depth, α, β)

    def negamax(self, state, depth, α, β):
        alphaOrig = α
        ...
        terminal = self.game.terminal_test(state)
        if terminal != 0:
            return terminal * inf
        if depth == 0:
            return self.eval_fn(state, self.game.color)
        ...
        for m in moves:
            ...
            child_v = self.negamax(next_state, depth-1, -β, -α)
